...
**`plot_varieties`** (Phase 2, added 2026-08-11; `area_acres`/`crop` added 2026-09-03) — varieties grown on a plot
`id` PK · `plot_id` FK→plots · `variety_name` string · `area_acres` decimal, nullable (the variety's share of the plot's total area — not the plot's full area; see below) · `crop` enum(`Grape`), not null, default `Grape` (forward-compat placeholder — see below) · `created_at`
...
*`area_acres` note: nullable because field measurement is imprecise and workers won't always subdivide acreage cleanly at registration time. Not enforced against `plots.area_acres` as a DB constraint — the UI warns (doesn't block) if a plot's variety areas sum to more than its stated total; an under-total is normal and unflagged (unallocated/non-varietal area within a plot is expected).*

*`crop` note — forward-compatibility hook, not a built feature:* the CEO has confirmed pomegranate and banana are planned once grapes are done, but their requirements are unknown and nothing for them is built here — no `crops` table, no configurable QC parameter set, no per-crop pack sizes or compliance types. `crop` exists only so `variety_name` isn't a free string with an unstated grapes-only assumption baked into every query — without it, a future pomegranate variety like "Bhagwa" would sit in the same column as "Thompson Seedless" with nothing to tell a query which crop it belongs to. Single enum value (`Grape`) today; the plot-variety form does not ask for it. **Unanswered, deliberately not guessed at:** does a second crop share this system's QC parameter set (fruit colour/TSS/thrips/bhuri/black spot/cercospora — all grape-specific observations), or need its own? Does it share the 4/4.5/5 Kg pack-size tiers, or need different ones? Does it share `ComplianceType` (EU/Non-Testing), or does a different compliance regime apply? None of these have real requirements yet — resolve when crop #2 is actually scoped, not before.

**`season_registrations`** (Phase 2) — the seasonal join on `plots`, via `plot_varieties`
`id` PK · `plot_id` FK→plots, kept denormalized (derivable via `plot_variety.plot_id`) · **`plot_variety_id` FK→plot_varieties, NOT NULL as of 2026-09-03** (alembic `9d2f6a1c8b3e` — migration written and reviewed, **not yet applied to the live DB** as of this writing) · ~~`season_year` integer (R11)~~ **`season_id` FK→seasons — still PENDING, not yet built** (unrelated to the `plot_variety_id` change; `season_year` remains the live constraint column) · `status` enum (see Section 4 state machine — now includes `Finished Goods QC Passed`/`Failed`, added 2026-08-11) · `registered_by` FK→users · `registered_at` timestamp · unique constraint on `(plot_variety_id, season_year)` (`uq_season_registrations_plot_variety_season`, alembic `9d2f6a1c8b3e`, replaces the old `(plot_id, season_year)`) — **one registration per variety per plot per season**
*Note: `plot_variety_id` moved from pending/nullable to required + the uniqueness key on 2026-09-03, as the first step of the multi-variety-per-plot frontend work — see `CLAUDE.md` and the alembic migration's own docstring for the backfill it runs (every existing plot's `variety` becomes a `plot_varieties` row if it doesn't have one already; every existing registration gets linked to it). `season_id`/`season_year` are a separate, still-unrelated pending change — don't conflate the two. Do not write new code that treats `season_year` as authoritative going forward for anything other than this uniqueness key (`CLAUDE.md` Section 12).*
