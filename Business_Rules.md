...
Status: **Frozen v1.5** (v1.4 + 2026-09-02 documentation-vs-code audit: R2/R7a's "MH is farmer-level, CEO confirmed" was found to contradict the live code — `farmers` has no MH column, `plots.mh_registration_number` does — and to contradict `PHASE_MAP.md`'s own tail "New information" section, which says the opposite of its own Section 7. Both rules are now marked UNRESOLVED rather than either being picked as the winner; see the rewritten R2/R7a below and `Open_Questions.md` Q16. Same pass: R7's "neither `plots` nor `harvests` has a variety column" claim was checked against the code and found wrong for `plots` specifically — `plots.variety` is live, required on the plot registration form, and read directly by the Weighing screen; R7 is rewritten to say so. v1.4 was: 2026-08-31 founder confirmation: rejection is a FIXED 7% company-wide, not a per-contract negotiated term and not capped/split against actual observed rejection — supersedes the "farmer absorbs up to contract %, exporter absorbs the rest" reading (R24, R28, R48 rewritten). v1.3 was: same-day follow-up, 2026-08-11: multi-variety plots corrected — variety lives on `plot_varieties`, not `harvests` as v1.2 said (R7, new R57), permissions are now phase-based via `user_phase_access` (R53 rewritten, new R58), farmer search fixed to include MH number (R3), Farmer Invoice formula clarified — no deductions beyond the contract-rejection cap (R48, now itself superseded, see above), Q4 is RESOLVED not reopened. v1.2 was: CEO confirmation round, 2026-08-11: Season Management is now a real module (R55), MH number reversed back to farmer-level (R2, R7a) — **this specific claim is what v1.5 above now marks unresolved**, multi-variety plots reversed (R7), Packaging Supervisor role added (R56), Purchase Order module dropped, Finished Goods QC position confirmed (R21); remaining open items in `Open_Questions.md` — note that file still says Q4 "REOPENED", not yet updated to match)
Last updated: 2026-09-02, documentation-vs-code audit on MH registration number and plot variety (previously: 2026-08-31, founder confirmation on the rejection percentage; before that, 2026-08-11 same-day follow-up round; before that, 2026-08-11 CEO confirmation round; before that, Aug 2026 CEO answers on variety-per-plot and rejection-split; originally from Phase 0 discovery sessions)
...
2. ~~Each plot must have a unique mh_registration_number (APEDA plot-level registration). No MH identifier exists at the farmer level.~~ ~~**REWRITTEN 2026-08-11 — reversed:** A farmer has a unique MH registration number (`mh_number`)...~~ **UNRESOLVED — re-flagged 2026-09-02, do not treat either version above as settled.** The 2026-08-11 rewrite above was written up as CEO-confirmed and farmer-level. It wasn't actually settled: `PHASE_MAP.md`'s own Section 7 narrates that same farmer-level story, while that document's separate tail "New information" section says the opposite (MH is per plot, three per farmer) — the two halves of one document have never been reconciled. **The running code is per-plot**, checked directly: `backend/app/models/farmer.py` has no MH column at all; `backend/app/models/plot.py:51` has `mh_registration_number = Column(String, unique=True, nullable=True)`. That the code currently matches the "per-plot" reading is not proof that reading is correct — it was never re-checked against this rule's farmer-level claim. No APEDA/GrapeNet registration certificate has ever been seen from the client; this is blocked on that document, not on more discussion. **Cost if the resolution ends up farmer-level (i.e., code must change):** an Alembic migration dropping `plots.mh_registration_number` and adding `farmers.mh_number` (unique, nullable), plus rework of farmer search and of the plot registration screen. See `CLAUDE.md` Discovery 8, `PHASE_MAP.md` Section 7, `Open_Questions.md` Q16.
...
7. ~~A plot is associated with one grape variety at a time. A farmer can grow multiple varieties, but each variety is registered as a **separate plot**. ✅ CEO confirmed (Aug 2026): one plot = one variety...~~ **REWRITTEN 2026-08-11, corrected same day:** A plot may contain multiple grape varieties. This reverses the Aug 2026 resolution above. ✅ **CEO confirmed, RESOLVED** (`Open_Questions.md` Q4 — settled, not open; see R57 for how this rewired the pipeline). *Same-day correction:* an earlier pass of this rule said the variety is recorded on the harvest record (`harvests.variety`) — that was wrong. Variety is registered on the plot via the `plot_varieties` table, and each variety gets its own independent registration/QC/lab/contract/harvest pipeline. `harvests` genuinely has no variety column — confirmed, still true. See R57.
    **The "neither `plots` nor `harvests` has a variety column" half of the previous sentence was wrong, corrected 2026-09-02.** `plots` does have one: `backend/app/models/plot.py:52`, a live `variety` string column. It is not unused leftover — `frontend/src/features/plots/pages/PlotRegistrationPage.tsx` has it as a *required* field on the plot registration form, `create_plot` persists whatever is submitted, and `backend/app/api/v1/routers/weighing.py:77` reads `reg.plot.variety` directly to show variety on the Weighing screen (not via `season_registration → plot_variety` as this rule and the phase specs describe). `plot_varieties` has full backend support but **no frontend screen** anywhere in the codebase — nothing creates or edits a `plot_varieties` row today, so `season_registrations.plot_variety_id` is always null in practice. In effect, the multi-variety pipeline this rule describes is not yet actually reachable by a user; the single-variety-per-plot mechanism it was meant to replace is still what's live. This is a gap to close (build the `plot_varieties` UI and move `weighing.py:77` off `plot.variety`), not a fact to keep asserting as already true.
7a. ~~The MH registration number is a plot-level APEDA identifier only. It must never be stored or referenced at the farmer level.~~ ~~**REWRITTEN 2026-08-11 — reversed accordingly:** The MH registration number is a farmer-level identifier only...~~ **UNRESOLVED — re-flagged 2026-09-02, see rewritten R2 above for the full detail.** Neither "plot-level only" nor "farmer-level only" is confirmed. The code currently stores it on `plots` (`mh_registration_number`, unique, nullable) and has no farmer-level column; whether that's correct business practice or needs to move is blocked on an APEDA/GrapeNet certificate nobody has seen yet. Do not state either direction as a rule until R2 is actually resolved.
