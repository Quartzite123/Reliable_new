...
>
> **Last updated:** 2026-09-02 — documentation-vs-code audit (`docs/DOSSIER_FACTS.md`, cross-checked against `PROJECT_DOSSIER.md`) found this file stating three things as settled fact that the running code contradicts. No application code was changed to fix this — the code is what's live; the docs were stale. Corrected here: (1) **Discovery 8 / MH registration number** — was asserted as farmer-level, CEO-confirmed; the code has no `farmers.mh_number` column at all, only `plots.mh_registration_number`, and `PHASE_MAP.md` already had a self-contradiction on this exact point (its Section 7 narrates farmer-level, its tail "New information" section says plot-level). Now marked **UNRESOLVED** here, in `Business_Rules.md` R2/R7a, and in `PHASE_MAP.md` Section 7 (which was already flagged there as of 2026-09-02) — none of the four documents wins by default; see the rewritten Discovery 8 below and `Open_Questions.md` Q16. (2) **The Section 12 "never store variety on plots" rule** — `plots.variety` is not dead debt quietly left behind; it's a required field on the live Plot Registration screen and is read directly by `weighing.py:77`, while `plot_varieties` (the documented replacement) has no frontend screen at all. Rule intent stands, current code violates it in a load-bearing way — see the rewritten Section 12 bullet. (3) **Section 10's Finished Goods QC statuses** — was stated in the present tense as "now real values in the `season_registrations.status` enum." They aren't; `RegistrationStatus` has 13 values, neither Finished Goods QC one among them. Corrected below. See Section 6 (roles/login) and Section 12 (never-do list) for the older 2026-09-01 security-audit entry, kept below.
>
> **Previously:** 2026-09-01 — security audit + admin user-management overhaul folded in: login timing/rate-limiting/token-revocation fixes, CORS pinned, password minimum raised to 12; a new **Users** phase splits user management out of Admin (with a full backend-enforced boundary — see `backend/app/services/user_admin_guard.py`), a **Reports & Documents** placeholder phase was added (same pattern as Finished Goods QC), and password policy is now explicit: admin-set passwords are permanent, there is no self-service or email-based reset, and `scripts/seed_admin.py` is the documented break-glass recovery procedure. See Section 6 (roles/login) and Section 12 (never-do list) below.
>
...
### Discovery 8: MH registration number — UNRESOLVED (previously stated as "belongs to the farmer, reversed 2026-08-11"; re-flagged 2026-09-02)
This was previously written up here as settled: "MH registration number belongs to the farmer, not the plot. One MH number per farmer... CEO confirmation overrides the documentation research. Follow business practice: `farmers.mh_number`." **It is not settled, and that text was wrong to present as resolved.** `Business_Rules.md` and `PHASE_MAP.md` Section 7 all narrate the same farmer-level story as CEO-confirmed — but `PHASE_MAP.md`'s own tail "New information" section (added from a later, separate finding) says the opposite: MH is per plot, three per farmer, and the farmer-level move should be reverted. The two halves of that one document have never been reconciled against each other.

**What the code actually does, checked directly (2026-09-02):** `backend/app/models/farmer.py`'s `Farmer` class has no MH column of any kind. `backend/app/models/plot.py:51` has `mh_registration_number = Column(String, unique=True, nullable=True)`. The running system is per-plot. That the code currently agrees with the "New information" finding is not the same as that finding being confirmed correct — it was never checked against the farmer-level narrative it contradicts, and no one has re-derived which of the two is actually right.

**No APEDA/GrapeNet registration certificate has ever been seen from the client.** Nothing in hand settles whether MH numbers are issued per-farmer or per-plot in Reliable Fresh's actual practice — this is blocked on that one document. Do not treat either version as authoritative; do not "fix" the code to match either document without it.

**Cost if the resolution goes the other way from the code (i.e., if MH turns out to be farmer-level):** an Alembic migration to drop `plots.mh_registration_number` and add `farmers.mh_number` (unique, nullable), plus rework of the farmer search flow (which would need to search a new farmer-level field instead of/alongside the current plot-level one) and of plot registration (which would lose the MH field entirely). If the resolution instead confirms the code's current per-plot model, the cost is limited to fixing the documentation — no schema change. Either way, nothing should be changed in the code on the strength of this section alone. See `Business_Rules.md` R2/R7a, `PHASE_MAP.md` Section 7, `Open_Questions.md` Q16.
...
3. **Business key vs database key** — the database uses invisible internal IDs as primary keys. Plot Number and MH Number are unique constraints for integrity but never exposed as the "primary key" to users. *(Which table MH Number actually lives on is unresolved as of 2026-09-02 — see Discovery 8 above. Don't restate `farmers.mh_number` as settled; the live column is `plots.mh_registration_number`.)*
...
**Position of Finished Goods QC confirmed 2026-08-11** (was a future/unplaced status): it sits between Palletised and Pre-Cooled, not after Pre-Cooled as an earlier draft speculated — that part is genuinely settled and nothing below changes it.

**Corrected 2026-09-02 — the sentence that used to follow was wrong.** This section previously stated, present tense, that `'Finished Goods QC Passed'` and `'Finished Goods QC Failed'` "are now real values in the `season_registrations.status` enum, not future placeholders." Checked directly against `backend/app/core/enums.py::RegistrationStatus`: it has exactly 13 values (`Registered` through `Pre-Cooled`), and neither Finished Goods QC value is among them. A repo-wide grep for `finished_goods_qc`/`FinishedGoodsQC` turns up exactly one hit in the entire backend — the `PhaseKey.FINISHED_GOODS_QC` phase placeholder in the same `enums.py` file, which gates a screen, not a status. There is no `finished_goods_qc` table, no model, no status values, and no `status_machine.py` guard for this stage. It is blocked on the CEO's format document, same as it always was — only the *position* is confirmed, not the build.

**The frontend disagrees with the backend on this.** `frontend/src/types/season.ts`'s `SeasonRegistrationStatus` type DOES include `'Finished Goods QC Passed'`/`'Finished Goods QC Failed'` as literal values, even though the backend enum that's supposed to be its source of truth has no such values and no endpoint can ever produce them. Anyone building against the frontend type alone would reasonably assume this stage exists; it doesn't. This mismatch should be resolved (either drop the values from the frontend type until the backend has them, or treat their presence as a standing TODO) rather than left silently disagreeing.
...
- ~~Never store MH number on the plot — it belongs to the farmer (`farmers.mh_number`).~~ **UNRESOLVED as of 2026-09-02 — do not enforce this rule in either direction.** This bullet used to assert farmer-level as settled; it wasn't, and the live code (`plots.mh_registration_number`, not `farmers.mh_number`) already contradicts it. See the rewritten Discovery 8 in Section 3 for the full picture, `Business_Rules.md` R2/R7a, `PHASE_MAP.md` Section 7, and `Open_Questions.md` Q16. Blocked on an APEDA/GrapeNet registration certificate from the client — nobody has seen one. Don't "fix" the code toward either table on the strength of this document alone.
...
- Never store variety directly on `plots` or `harvests` — variety is registered per plot via `plot_varieties` (added 2026-08-11); `harvests` inherits it through `season_registration → plot_variety`, never its own column (see `Business_Rules.md` R57). **The rule still describes the intended design. The code does not follow it, and this is not inert leftover debt — checked 2026-09-02:** `backend/app/models/plot.py:52` has a live `variety` string column; `frontend/src/features/plots/pages/PlotRegistrationPage.tsx` has it as a *required* field on the plot registration form (`register('variety')`, validated required); `backend/app/api/v1/routers/plots.py`'s `create_plot` persists it via `Plot(**body.model_dump())`; and `backend/app/api/v1/routers/weighing.py:77` reads `reg.plot.variety` directly to populate the Weighing screen's reference panel — not `season_registration → plot_variety` as the design and every phase-spec screen note claim. Meanwhile `plot_varieties` has a full backend (model, router, schemas) but **no frontend screen anywhere** — nothing in `frontend/src/features/plots/` or elsewhere creates, edits, or displays a `plot_varieties` row, and `season_registrations.plot_variety_id` is consequently always null in practice, since nothing ever sends it. In effect, the system today still runs on the pre-2026-08-11 single-variety-per-plot model at the one place that matters (data entry and at least one downstream read), while the documented multi-variety replacement sits unused. Do not remove the `plots.variety` column to match this rule without first replacing what `weighing.py:77` and `PlotRegistrationPage.tsx` depend on it for — that would break variety display on Weighing and the plot registration form with no replacement path wired up yet.
